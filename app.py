...
@gs.on("json_message")
def socket_worker(message: Dict[str, str]) -> None:
...
def updates_check() -> None:
    ...
    while gs.count_of_people_in_lobby() > 1:
        ...
        while not gs.is_player_connected(player_2):
            player_2 = lobby_queue.get()
...
def start_game(player_1: str, player_2: str) -> None:
    ...
    gs.start_game(player_1, players[player_2].__dict__)
    gs.start_game(player_2, players[player_1].__dict__)
    update_rating_table()
...
def game_progress(game_id: str) -> None:
    """Проведение игры
